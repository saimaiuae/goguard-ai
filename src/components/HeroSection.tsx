...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen pt-28 pb-20 flex items-center overflow-hidden" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <canvas
        ref={canvasRef}
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12">
          <div className="flex-1 max-w-xl">
            <div className="flex items-center gap-2 mb-6 bg-white/5 py-2 px-3 rounded-full w-fit animate-fade-in" style={{ animationDelay: '0.2s' }}>
              ...
              <span className="text-sm font-medium text-white/80">{t('hero.aiPowered')}</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 animate-fade-in" style={{ animationDelay: '0.6s' }}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/70">{t('hero.aiProtection')}</span>
              </div>
              <div className="flex items-center gap-2">
                <Monitor size={18} className="text-athlete-blue" />
                <span className="text-sm text-white/70">{t('hero.realTime')}</span>
              </div>
              <div className="flex items-center gap-2">
                <Activity size={18} className="text-athlete-blue" />
                <span className="text-sm text-white/70">{t('hero.performanceInsights')}</span>
              </div>
            </div>
          </div>
                ...
                <div className="pt-8 pb-4 px-4">
                  <div className="aspect-video bg-athlete-darker rounded-md overflow-hidden relative">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="p-6 text-center">
                        ...
                        <div className="font-medium text-white">{t('hero.processingVideo')}</div>
                        <div className="text-xs text-white/50 mt-1">{t('hero.analyzingMovements')}</div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    ...
                    <div className="neo-panel p-3 flex flex-col">
                      <span className="text-xs text-white/50 mb-1">{t('hero.fatigueIndex')}</span>
                    ...
                    <div className="neo-panel p-3 flex flex-col">
                      <span className="text-xs text-white/50 mb-1">{t('hero.injuryRisk')}</span>
                      <div className="text-lg font-semibold text-athlete-red mb-1">{t('hero.medium')}</div>
                      <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                    ...
                    <div className="neo-panel p-3 flex flex-col">
                      <span className="text-xs text-white/50 mb-1">{t('hero.sprintCount')}</span>
                      <div className="text-lg font-semibold text-white">12</div>
                      <span className="text-xs text-athlete-blue mt-1">{t('hero.fromAvg')}</span>
                    </div>
                    
                    <div className="neo-panel p-3 flex flex-col">
                      <span className="text-xs text-white/50 mb-1">{t('hero.playTime')}</span>
                      <div className="text-lg font-semibold text-white">24:18</div>
                      <span className="text-xs text-white/50 mt-1">{t('hero.minutes')}</span>
                    </div>
                  </div>
                </div>
              </div>
