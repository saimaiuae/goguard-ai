...
const RecentVideo = ({ title, duration, date, players, isNew }: RecentVideoProps) => {
  return (
    <div className={`neo-panel p-4 rounded-lg flex gap-4 ${isNew ? 'border border-athlete-blue/30' : ''}`}>
      <div className="w-12 h-12 bg-athlete-blue/20 rounded-md flex-center flex-shrink-0">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          {isNew && (
            <span className="px-1.5 py-0.5 bg-athlete-blue text-white text-xs rounded-full">New</span>
          )}
        </div>
